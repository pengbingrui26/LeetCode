...
public:
    ...
    {
        int output = std::min(height[1], height[0]) * 1;
        for (int i = 0; i < height.size() - 1; i++)
        {
            int max_idx = i + 1;
            for (int j = max_idx; j < height.size(); j++)
            {
                if (height[j] > height[max_idx])
                {
                    max_idx = j;
                }
            }
            // int hind = max_idx;
            int max_vol = std::min(height[max_idx], height[i]) * (max_idx - i);
            for (int hind = max_idx; hind < height.size(); hind++)
            {
                int this_vol = std::min(height[hind], height[i]) * (hind - i);
                if (this_vol > max_vol)
                {
                    max_vol = this_vol;
                }
...
public:
    int maxArea(std::vector<int> &height)
    {
        ...
        for (int front = 0; front < height.size() - 1; front++)
        {
            int this_max_idx = HashList[front];
            int max_vol = std::min(height[front], height[this_max_idx]) * (this_max_idx - front);
            for (int j = this_max_idx; j < height.size(); j++)
            {
                int tmp_vol = std::min(height[front], height[j]) * (j - front);
                if (tmp_vol > max_vol)
                {
                    max_vol = tmp_vol;
                }
...
public:
    int maxArea(std::vector<int> &height)
    {
        int output = std::min(height[1], height[0]) * 1;
        std::unordered_map<int, int> HashList;
        HashList[height.size() - 2] = height.size() - 1;

        for (int front = height.size() - 2; front >= 0; front--)
        {
            int max_idx;
            if (front == height.size() - 2)
            {
                max_idx = HashList[front];
            }
            else
            {
                if (height[front + 1] >= HashList[front + 1])
                {
                    max_idx = front + 1;
                }
                else
                {
                    max_idx = HashList[front + 1];
                }
            }
            HashList[front] = max_idx;
            int this_max_idx = max_idx;
            int max_vol = std::min(height[front], height[this_max_idx]) * (this_max_idx - front);
            for (int j = this_max_idx; j < height.size(); j++)
            {
                int tmp_vol = std::min(height[front], height[j]) * (j - front);
...
void test03()
{
    ...
    height = {59, 15, 23, 55, 30, 47, 61, 74, 86, 25, 42, 40, 21, 0, 87, 79, 45, 42, 0, 47};
    auto it3 = solution3.maxArea(height);
    auto it4 = solution4.maxArea(height);
